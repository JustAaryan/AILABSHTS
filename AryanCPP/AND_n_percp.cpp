...
void generate_and_truth_table(int n, vector<vector<int>>& X, vector<int>& Y) {
    int rows = pow(2, n);
    for (int i = 0; i < rows; ++i) {
        vector<int> row(n);
        int val = i, and_result = 1;
        for (int j = n - 1; j >= 0; --j) {
            row[j] = val % 2;
            val /= 2;
            and_result &= row[j];
        ...
        X.push_back(row);
        Y.push_back(and_result);
    }
}

int main() {
    int n; 
    cout<<"Include the no of input";
    cin>>n;
    vector<vector<int>> X;
    vector<int> Y;
    generate_and_truth_table(n, X, Y);

    Perceptron p(n);
    cout << "Training Perceptron for " << n << "-input AND Gate...\n";
    p.train(X, Y);
